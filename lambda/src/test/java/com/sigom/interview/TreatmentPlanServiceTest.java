...
public class TreatmentPlanServiceTest {
        ...
        for (Disease d : TestHelper.diseaseList()) {
            ...
            switch (d.getName()) {
                ...
                case "Peripheral Neuropathy":
                    expected.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Capsaicin Cream")).findFirst().get(), BigDecimal.valueOf(34));
                    expected.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Lidocaine Patches")).findFirst().get(), BigDecimal.valueOf(136));
                    break;
                case "Dural Arteriovenous Fistulae":
                    ...
                    break;
                case "Common Cold":
                    expected.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Echinacea")).findFirst().get(), BigDecimal.valueOf(272));
                    expected.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Acetaminophen")).findFirst().get(), BigDecimal.valueOf(136));
    ...
    public void treatmentPlanForPatient() throws Exception {
        ...
        expectedMedications.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Acetaminophen")).findFirst().get(), BigDecimal.valueOf(136));
        expectedMedications.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Ibuprofen")).findFirst().get(), BigDecimal.valueOf(340));
        expectedMedications.put(TestHelper.medications().stream().filter(m -> m.getName().equals("Echinacea")).findFirst().get(), BigDecimal.valueOf(272));
